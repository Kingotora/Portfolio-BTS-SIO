...
function initConsole() {
  ...
  const manuals = {
    ...
    'music': 'Toggle background Lo-Fi music (Lofi Girl). Usage: music',
    ...
    'weather': 'Display current weather conditions. Usage: weather [city]',
  ...
  const startInvaders = () => {
    ...
    const finishGame = (win) => {
      ...
      iCanvas.remove(); // Clean up canvas
      if (win) {
        print('VICTORY! Mission Accomplished.', 'success');
        print('Final Score: ' + iScore, 'info');
      } else {
        print('GAME OVER. The aliens have taken over.', 'error');
        print('Final Score: ' + iScore, 'warn');
      }
      isGameRunning = false; // Release lock
      prompt.style.display = 'flex';
      input.focus();
    };

    // Use input listener only to abort
  ...
  const COMMANDS = {
    ...
    'weather': (args) => {
      const city = args.slice(1).join(' ') || 'Server Room';
      const cities = ['Server Room', 'Paris', 'Tokyo', 'New York', 'Mars Colony'];
      const icons = ['🌡️', '☁️', '☀️', '❄️', '🚀'];
      // Deterministic pseudo-random based on city string length for consistency
      const r = city.length % icons.length;
      const temp = (city === 'Server Room') ? '19°C' : Math.floor(Math.random() * 35 - 5) + '°C';

      setTimeout(() => {
        print(`${icons[r]} ${city}: ${temp}`, 'info');
      }, 500);
    },
    ...
    'music': () => {
      if (!window.lofiAudio) {
        window.lofiAudio = new Audio('https://stream.zeno.fm/0r0xa792kwzuv');
        window.lofiAudio.volume = 0.5;
      }
      if (window.lofiAudio.paused) {
        window.lofiAudio.play().then(() => {
          print('Playing: Lo-Fi Hip Hop Radio 🎵', 'success');
        }).catch(e => print('Audio blocked. Click document first.', 'error'));
      } else {
        window.lofiAudio.pause();
        print('Music paused. ⏸️', 'warn');
      }
    },
    'make': (args) => {
...
// =========================================
//  INTERACTIVE FEATURES (Drag & Sound)
// =========================================
document.addEventListener('DOMContentLoaded', () => {
